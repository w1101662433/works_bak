...
class ReceiveMsgSocketServer(socketserver.BaseRequestHandler):
    ...
    def msg_callback(msg):
        try:
            nick_name = get_chatroom_user_nick_name(server_port, msg.get('sender'), msg.get('wxid'), )
            json_data = {
                'msgid': msg.get('msgid'),
                'wxid': msg.get('wxid'),
                'nickname': nick_name,
...
def test_send_msg(test_port):
    ...
    if post_wechat_http_api(APIS.WECHAT_IS_LOGIN, port=test_port)["is_login"] == 1:
        show_log(post_wechat_http_api(APIS.WECHAT_GET_SELF_INFO, port=test_port))
        ...
        show_log(post_wechat_http_api(APIS.WECHAT_CONTACT_GET_LIST, port=test_port))
        data = {"wxid": "filehelper"}
        show_log(post_wechat_http_api(APIS.WECHAT_CONTACT_CHECK_STATUS, data=data, port=test_port))
...
def get_newest_msg(test_port, limit=100):
    dbs = post_wechat_http_api(APIS.WECHAT_DATABASE_GET_HANDLES, port=test_port)
    show_log('dbs', dbs)
    db_handle = [i for i in dbs['data'] if i['db_name'] == 'MSG0.db'][0]['handle']
    sql = f"SELECT * FROM MSG  order by localId desc limit {limit}"
    data = {"db_handle": db_handle, "sql": sql}
    res = post_wechat_http_api(APIS.WECHAT_DATABASE_QUERY, data=data, port=test_port)
    return res['data']


def get_chatroom_user_nick_name(test_port, chatroom_id='38974356001@chatroom', wxid='wxid_qk4h0a9xn8eu21'):
    res = post_wechat_http_api(APIS.WECHAT_CHATROOM_GET_MEMBER_NICKNAME, port=test_port, data={"chatroom_id": chatroom_id, "wxid": wxid})
    return res.get('nickname', '')


def send_image(test_port):
    post_wechat_http_api(APIS.WECHAT_LOG_START_HOOK, port=test_port)
    if post_wechat_http_api(APIS.WECHAT_IS_LOGIN, port=test_port)["is_login"] == 1:
        show_log(post_wechat_http_api(APIS.WECHAT_GET_SELF_INFO, port=test_port))

        data = {"receiver": 'filehelper', "img_path": r"./微信截图_20231123085100.png"}
        post_wechat_http_api(APIS.WECHAT_MSG_SEND_IMAGE, data=data, port=test_port)
...
if __name__ == '__main__':
    ...
    start_listen(pids[0], server_port)
    send_image(server_port)
    os._exit(0)
    # res = post_wechat_http_api(APIS.WECHAT_MSG_START_HOOK, server_port, {'port': server_port - 1})
    # show_log('res', res)
    # start_socket_server(server_port - 1)
